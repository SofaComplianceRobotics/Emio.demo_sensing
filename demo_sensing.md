...
|![Publishing the force using ROS2, and piloting the turtle sim.](assets/labs/demo_sensing/data/images/emio-sensing-turtlesim.png){width=80%}|
|:--------------------------------------------------------------------:|
...
3. Launch the simulation by clicking the Run SOFA button below.
On the GUI, select the window __Windows>Input/Output__. From this window __Input/Output__, in the output section, select the topic __Sensor/Force__ and click toggle the __Publish__ button (see image above).
...
|![Publishing the force using ROS2.](assets/labs/demo_sensing/data/images/emio-sensing-publishforce.png){width=80%}|
|:--------------------------------------------------------------------:|
|**Image 1. First simulation, publishing the force using ROS2.**                               |

|![Subscription.](assets/labs/demo_sensing/data/images/emio-sensing-teleoperation.png){width=80%}|
|:--------------------------------------------------------------------:|
...
1. In a terminal, from the demo_sensing directory, run the gateway script:
    ...
    This script subscribes to the topics __Sensor/Force__ and __TCP/Frame__ and publishes on the topic __TCPTarget/Frame__.

2. In the first simulation (Run SOFA button below), publish the force on the topic __Sensor/Force__ and the TCP position on the topic __TCP/Frame__ by first clicking the corresponding checkboxes, and then clicking the button __Publish__ in the window __Windows>Input/Output__ (see Image 1.).
#runsofa-button("assets/labs/demo_sensing/demo_sensing.py")  
...
Select "IOWindow" from the menu in the 3D view, this will allow the window __Windows>Input/Output__ to control the TCP target in the simulation (see Image 2.). 
